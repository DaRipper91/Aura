...
; General
Name "Aura"
OutFile "dist\Aura_Setup_v1.0.6.exe"
...
Section "Install"
    ...
    File "dist\Aura_v1.0.6_Lite.exe"
    
    ; Create uninstaller
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    
    ; Create Shortcuts
    CreateShortcut "$DESKTOP\Aura.lnk" "$INSTDIR\Aura_v1.0.6_Lite.exe"
    CreateDirectory "$SMPROGRAMS\Aura"
    CreateShortcut "$SMPROGRAMS\Aura\Aura.lnk" "$INSTDIR\Aura_v1.0.6_Lite.exe"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Aura" "DisplayIcon" "$INSTDIR\Aura_v1.0.6_Lite.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Aura" "Publisher" "DaRipper91"
SectionEnd

Section "Uninstall"
    Delete "$INSTDIR\Aura_v1.0.6_Lite.exe"
